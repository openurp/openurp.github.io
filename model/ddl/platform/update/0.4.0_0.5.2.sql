...
update usr.dimensions set name=substring(name,1,length(name)-1) where name like '%s';
--这一句需要根据提示出来的项目ID进行更新
select trim(source) from usr.dimensions where name='project';
update usr.user_profiles_properties upp set value='??' where exists(select * from usr.dimensions d where d.id=upp.dimension_id and d.name='project');

update cfg.apps set name='platform-portal' where name='platform-adminapp';

update cfg.apps set url=replace(url,'/platform/admin',''),base=replace(base,'/platform/admin','') where name='platform-portal';
update cfg.apps set url=replace(url,'{openurp.webapp}','{webapp}'),base=replace(base,'{openurp.webapp}','{webapp}');

update se.func_resources set name='/admin'||name where app_id =(select id from cfg.apps where name='platform-portal');
update se.func_resources set enabled=false where name='/admin/user/user';

update se.func_resources set name='/user'||name where app_id =(select id from cfg.apps where name='platform-userapp');

update se.func_resources set app_id=(select id from cfg.apps where name='platform-portal') where app_id =(select id from cfg.apps where name='platform-userapp');
update se.menus set app_id=(select id from cfg.apps where name='platform-portal') where app_id =(select id from cfg.apps where name='platform-userapp');

--删除原有的portal,platform-userapp
delete from usr.roots where app_id =(select id from cfg.apps where name='edu-portal');
delete from cfg.data_sources where app_id =(select id from cfg.apps where name='edu-portal');

delete from se.func_resources where app_id =(select id from cfg.apps where name='edu-portal');
delete from se.menus  where app_id =(select id from cfg.apps where name='edu-portal');

delete from cfg.apps where name='edu-portal';

delete from usr.roots where app_id =(select id from cfg.apps where name='platform-userapp');
delete from cfg.data_sources where app_id =(select id from cfg.apps where name='platform-userapp');
delete from se.func_resources where app_id =(select id from cfg.apps where name='platform-userapp');
delete from se.menus  where app_id =(select id from cfg.apps where name='platform-userapp');
delete from cfg.apps where name='platform-userapp';

--一下写上系统的名称
update cfg.domains set title='xx系统';

--升级blob
drop view blob.users;
create  view blob.users as select domain_id,name,secret as key from cfg.apps;
create or replace view blob.domains as select id,hostname from cfg.domains;

alter table blob.blob_metas  add domain_id int4;
update blob.blob_metas set domain_id=(select min(id) from cfg.domains);
alter table blob.blob_metas alter domain_id set not null;


alter table blob.profiles  add domain_id int4;
update blob.profiles set domain_id=(select min(id) from cfg.domains);
alter table blob.profiles alter domain_id set not null;

update blob.profiles  set users =replace(users,'platform-adminapp','platform-portal');
